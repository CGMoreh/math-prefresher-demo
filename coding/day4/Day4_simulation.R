...
sample(c("Head", "Tail"), size = 2000, prob = c(0.5, 0.5), replace = TRUE) %>% table
...
sample(c("Head", "Tail"), size = 200, prob = c(0.2, 0.8), replace = TRUE) %>% table
...
# `sample`, but is a continuous rather than discrete system.  
...
rbinom(1000, 1000, prob = .5) %>% qplot
pbinom(500, 1000, prob = .5)
dbinom(500, 1000, prob = .5)
...
mean(cen10$race != "White")

# (b) Setting a seed to `02138`, sample 100 respondents from this sample. What is the proportion of 
...
set.seed(02138)
sample_n(cen10, 100) -> cen10_sample

mean(cen10_sample$race != "White")
...
for(i in 1:20){
  sample_n(cen10, 100) -> cen10_sample
  mean(cen10_sample$race != "White") -> collector1[i]
}
...
cen10$propensity <- ifelse(cen10$race == "White", 1,0.9)

collector <- c()
set.seed(02138)

for(i in 1:20){
  sample_n(cen10, 100, weight = propensity) -> cen10_sample
  mean(cen10_sample$race != "White") -> collector[i]
}
...
for(i in 1:20){
  sample_n(cen10, 10000, weight = propensity, replace = TRUE) -> cen10_sample
  mean(cen10_sample$race != "White") -> collector3[i]
}
